...
module.exports = class PetController {
    ...
    //Função de agendamento de visita
    static async schedule(req, res) {
        ...
        //Verificar se o pet é meu (não pode agendar visita para o próprio pet)
        const token = getToken(req)
        const user = await getUserByToken(token)

        if(pet.user._id.equals(user._id)) {
            res.status(422).json({ message: "Não é permitido agendar visita com o próprio pet" })
            return
        }

        //Checar se o usuário já tem uma visita agendada
        if(pet.adopter) {
    ...
    static async concludeAdoption(req, res) {
        ...
        res.status(200).json({message: "Parabéns! O ciclo de adoção foi finallizado com sucesso!"})
